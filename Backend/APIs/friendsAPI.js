  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (!requesterId || !recipientId || requesterId === recipientId)
      return res.status(400).send({
        message:
          "RecipientId is required! (Verify if requesterId === recipientId)",
      });
    ...
    if (existingRel) {
      return res.status(400).send({
        message: "A relationship already exists. (Already requested)",
...
friendship.get(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const onlyAccepted = req.user._id !== userId;
    let conditions = [{ requester: userId }, { recipient: userId }];

    if (onlyAccepted) {
      conditions[0] = { ...conditions[0], status: "accepted" };
      conditions[1] = { ...conditions[1], status: "accepted" };
    }
    ...
    let friends = await FriendshipModel.find({
      ...
      .populate("requester recipient", "_id firstName lastName bio profilePic")
      .lean();
    friends = friends.map((friend) => {
      return {
        ...
        requester: {
          ...friend.requester,
          profilePic: baseURL + friend.requester.profilePic,
        },
        recipient: {
          ...friend.recipient,
          profilePic: baseURL + friend.recipient.profilePic,
        },
      };
    });
...
friendship.patch(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    console.log("requesterId: ", requesterId, " recipientId: ", recipientId);

    let updatedFriendship = await FriendshipModel.findOneAndUpdate(
      {
    ...
    console.log("Updated friendshhip: ", updatedFriendship);
...
friendship.delete(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const deletedRes = await FriendshipModel.findOneAndDelete({
      $or: [
        { requester: authUserId, recipient: friendId },
        { requester: friendId, recipient: authUserId },
      ],
    });
...
//       return res
//         .status(404)
//         .send({ message: "No users with pending status found!" });
//     } else {
