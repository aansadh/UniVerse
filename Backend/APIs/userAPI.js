...
users.get('/', expressAsyncHandler(async (req, res)=>{
...
users.get(['/profile', '/profile/:id'], expressAsyncHandler(async (req, res) => {
    const path = req.path
    ...
    let userDetails = await UserModel.findById(userId, {__v: false, password: false, email: false})
                                      .lean()
    if(!userDetails)
...
users.patch('/update', uploader('profilePic'), expressAsyncHandler(async (req, res) => {
    ...
    requestedUpdates = Object.fromEntries(
        Object.keys(requestedUpdates)
            .filter(key => allowedUpdates.has(key) && requestedUpdates[key] !== '' && requestedUpdates[key]) 
            .map(key => [key, requestedUpdates[key]])
    );

    fileName && (requestedUpdates.profilePic = fileName)

    const updateRes = await userModel.findByIdAndUpdate(req.user._id, {$set: requestedUpdates}, {runValidators: true})
    if(!updateRes)
        return res.status(404).send({message: "User not found!"})
...
users.get('/media/:fileName', expressAsyncHandler(async (req, res) => {
...
users.delete('/delete', expressAsyncHandler(async (req, res) => {
    ...
    res.send({message: `Account deleted successfully! Deleted message: ${postMsg}`})
}))

users.get('/search', expressAsyncHandler(async (req, res) => {
    ...
    requestedFields = Object.fromEntries(
        Object.keys(requestedFields)
            .filter(key => allowedFields.has(key) && requestedFields[key] !== '' && requestedFields[key])
            .map(key => [key, requestedFields[key]])
    )
    // The operation and security here might be improved by deciding projections.
    const reqUsers = await UserModel.find(requestedFields, {_id: true, firstName: true, lastName: true, profilePic: true})
    if(!reqUsers.length)
